...
public class Main {
    ...
    private static int seoul11_1_oppSnakeNum = 0;
    private static int seoul11_1_mySnakeNum = 0;

    static Opponent Me=new Opponent(){
        public int hunt(int opp, int turn, int opp_prev, int opp_last_pattern[][]) {
            // 이전에 상대방 뱀이 선택된 횟수 증가
            if(turn == 0) {
                seoul11_1_oppSnakeNum = 0;
                seoul11_1_mySnakeNum = 0;
            }
            if(opp_prev == 2) seoul11_1_oppSnakeNum++;
            else seoul11_1_oppSnakeNum = 0;
            if(seoul11_1_oppSnakeNum >= 4) return 2;

            if(seoul11_1_mySnakeNum >= 4) {
                seoul11_1_mySnakeNum = 0;
                return 1;
            }

            seoul11_1_mySnakeNum++;
            return 2;
        }
    };
